...
//! @brief      EPICS Generic Device Support for Event Generator Sequencers.
//!
//! @par Description:
//!   This file contains generic EPICS device support for event generator sequences.
//!
//!   A sequence can be represented in the EPICS data base by up to two optional records with
...
devInfoStruct* parseLink (
    ...
    EgDeclareSequence    DeclareSequence,    // Function to declare this sequence to the EVG.
...
epicsStatus EgSeqBoWrite (boRecord* pRec) {
    ...
    //=====================
    // If this is an asynchronous completion callback,
    // and if this is the record that started the update,
    // perform an asynchronous completion by setting PACT false
    // and VAL back to zero.
    //
    ...
    //=====================
    // Lock the Sequence object and try to start the update
    //
    pRec->pact = true;
    pDevInfo->waiting = false;
...
epicsStatus EgSeqMbboWrite (mbboRecord* pRec) {
    ...
    //
    pSequence->lock();
    status = pSequence->SetUpdateMode(static_cast<SequenceUpdateMode>(pRec->val));
    ...
    //
        ...
        pRec->pact = false;
        pRec->udf = false;
    }//end if Sequence is not updating
