...
\begin{itemize}
	...
	\end{verbatim}
	\item application specific templates. These are used to define application specific behavior, such as mapping of events (described in Section~\ref{sec:Event mapping}) or defining additional components plugged in to the EVR (eg. universal I/O modules). Each template accepts different macros that allow its configuration. The following EVR substitution file snippet shows how to blink a LED light on the EVR named \texttt{EVR0} in system \texttt{MTEST-VME-EVRTEST}, each time the event 1 is received. \newline\texttt{evr-specialFunctionMap.template} loaded from \texttt{\$(TEMPLATE\_DIR)} is used for this functionality.
	\begin{verbatim}
